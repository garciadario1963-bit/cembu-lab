...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .tl-root{padding:20px 0 30px;}
  .tl-header{padding:0 24px 16px;border-bottom:1px solid var(--line);}
  .tl-controls{display:flex;flex-wrap:wrap;align-items:center;gap:14px;margin-bottom:14px;}
  .tl-search{background:var(--panel);border:1px solid var(--line);border-radius:3px;padding:6px 10px;color:var(--ivory);font-family:var(--sans);font-size:12.5px;width:200px;outline:none;}
  .tl-search::placeholder{color:var(--muted);}
  .tl-zoom{display:flex;align-items:center;gap:5px;font-family:var(--mono);font-size:11.5px;color:var(--ivory-dim);}
  .tl-btn{background:var(--panel);border:1px solid var(--line);color:var(--ivory);width:24px;height:24px;border-radius:3px;cursor:pointer;font-family:var(--mono);font-size:13px;line-height:1;display:flex;align-items:center;justify-content:center;}
  .tl-btn:hover{border-color:var(--brass-dim);color:var(--brass);}
  .tl-btn.wide{width:auto;padding:0 9px;font-family:var(--sans);font-size:11.5px;}
  .tl-legend{display:flex;flex-wrap:wrap;gap:5px;}
  .tl-tag{display:flex;align-items:center;gap:5px;font-size:10.5px;color:var(--ivory-dim);background:var(--panel);border:1px solid var(--line);border-radius:20px;padding:3px 9px 3px 7px;cursor:pointer;user-select:none;transition:opacity .15s;}
  ...
  .tl-inner{position:relative;padding-left:24px;}
  .tl-axis-wrap{position:sticky;top:0;z-index:4;background:var(--ink);border-bottom:1px solid var(--line);}
  .tl-axis{position:relative;height:30px;margin-left:200px;}
  .tl-tick{position:absolute;top:0;bottom:0;border-left:1px solid var(--line-soft);}
  .tl-tick span{position:absolute;top:7px;left:3px;font-family:var(--mono);font-size:9.5px;color:var(--muted);white-space:nowrap;}
  .tl-tick.decade span{color:var(--ivory-dim);}
  .tl-rows{position:relative;margin-left:200px;}
  .tl-row{position:relative;height:34px;border-bottom:1px solid var(--line-soft);transition:opacity .15s;display:flex;align-items:stretch;}
  .tl-row.dim{opacity:.15;}
  .tl-row:hover{background:rgba(0,0,0,0.035);}
  .tl-row-label{position:sticky;left:0;z-index:2;flex:none;width:200px;margin-left:-200px;height:34px;display:flex;align-items:center;gap:6px;padding-right:8px;background:var(--ink);border-right:1px solid var(--line);}
  .tl-row-num{font-family:var(--mono);font-size:9.5px;color:var(--muted);width:16px;text-align:right;flex:none;}
  .tl-row-dot{width:6px;height:6px;border-radius:50%;flex:none;}
  .tl-row-name{font-size:11px;color:var(--ivory);line-height:1.2;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1 1 auto;min-width:0;}
  .tl-row-years{font-family:var(--mono);font-size:9.5px;color:var(--muted);flex:none;white-space:nowrap;}
  .tl-track{position:relative;height:34px;flex:none;}
  .tl-seg{position:absolute;top:6px;height:22px;border-radius:2px;display:flex;align-items:center;justify-content:center;overflow:hidden;cursor:default;}
  .tl-seg span{font-family:var(--mono);font-size:9px;letter-spacing:.04em;white-space:nowrap;padding:0 5px;color:#2B2A24;text-shadow:0 1px 0 rgba(255,255,255,.15);}
  ...
  .tl-today-label{position:sticky;top:30px;transform:translateX(6px);font-family:var(--mono);font-size:9.5px;color:var(--brass);white-space:nowrap;display:inline-block;}
  .tl-tooltip{position:fixed;pointer-events:none;background:var(--panel-2);border:1px solid var(--brass-dim);border-radius:4px;padding:9px 12px;max-width:280px;font-size:12px;line-height:1.5;color:#EAE3D2;z-index:50;opacity:0;transition:opacity .1s;box-shadow:0 10px 24px rgba(0,0,0,.18);}
  .tl-tooltip.show{opacity:1;}
  .tl-tooltip .tt-title{font-family:var(--serif);font-size:13px;margin-bottom:3px;color:#EAE3D2;}
  .tl-tooltip .tt-range{font-family:var(--mono);color:#D8B972;font-size:10.5px;margin-bottom:5px;}
  .tl-tooltip .tt-desc{color:#A9A392;}
</style>
</head>
<body>
<div class="tl-root">
...
function buildRows(){
  ...
  today.style.left=yToX(TODAY)+'px';today.style.height=(EMPIRES.length*34)+'px';
...
function setZoom(newIdx,anchorYear){
  ...
  const centerYear=anchorYear!==undefined?anchorYear:MIN_YEAR+(scrollEl.scrollLeft+rect.width/2-200)/pxPerYear;
  zoomIdx=newIdx;pxPerYear=ZOOM_STEPS[zoomIdx];renderAll();
  scrollEl.scrollLeft=yToX(centerYear)-rect.width/2+200;
}
...
document.getElementById('tlGoToday').addEventListener('click',()=>{
  const scrollEl=document.getElementById('tlScroll');const rect=scrollEl.getBoundingClientRect();
  scrollEl.scrollLeft=yToX(TODAY)-rect.width/2+200;
});
...
buildLegend();syncLegend();renderAll();
        ...
        """
        components.html(timeline_html, height=1550, scrolling=True)
        st.markdown('</div>', unsafe_allow_html=True)
